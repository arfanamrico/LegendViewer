...
class LegendOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # ── State toggle jumlah sample & persentase ──
        ...
        # ── Opacity box utama sama dengan header (230 ≈ 90%) ──
        self.setStyleSheet("""
            QWidget#legendOverlay {
        ...
        self.header.setStyleSheet("""
            background: rgba(223, 230, 233, 230);
            border-radius: 4px;
        """)
        ...
        # =========================
        # TOMBOL TOGGLE: JUMLAH SAMPLE (N) & PERSENTASE (%)
        # =========================
        toggle_widget = QWidget()
        toggle_widget.setStyleSheet("background: transparent; border: none;")
        toggle_layout = QHBoxLayout(toggle_widget)
        toggle_layout.setContentsMargins(0, 0, 0, 0)
        toggle_layout.setSpacing(2)

        self.toggle_style_on = """
            QPushButton {
                background: #27ae60;
                color: white;
                border: none;
        ...
        self.btn_toggle_count = QPushButton("N")
        self.btn_toggle_count.setFixedSize(22, 20)
        self.btn_toggle_count.setCheckable(True)
        self.btn_toggle_count.setChecked(self.show_count)
        self.btn_toggle_count.setToolTip("Tampilkan/sembunyikan jumlah sample")
        self.btn_toggle_count.setStyleSheet(
            self.toggle_style_on if self.show_count else self.toggle_style_off
        )
        self.btn_toggle_count.clicked.connect(self.on_toggle_count)

        self.btn_toggle_pct = QPushButton("%")
        self.btn_toggle_pct.setFixedSize(22, 20)
        self.btn_toggle_pct.setCheckable(True)
        self.btn_toggle_pct.setChecked(self.show_percentage)
        self.btn_toggle_pct.setToolTip("Tampilkan/sembunyikan persentase")
        self.btn_toggle_pct.setStyleSheet(
            self.toggle_style_on if self.show_percentage else self.toggle_style_off
        )
        self.btn_toggle_pct.clicked.connect(self.on_toggle_percentage)

        toggle_layout.addWidget(self.btn_toggle_count)
        toggle_layout.addWidget(self.btn_toggle_pct)
        ...
        # ── Samakan opacity dengan box utama ──
        ...
        self.scroll_content.setStyleSheet("background: rgba(255, 255, 255, 230); border: none;")
    ...
    # ======================================================
    # TOGGLE JUMLAH SAMPLE
    # ======================================================
    def on_toggle_count(self):
        self.show_count = self.btn_toggle_count.isChecked()
        self.btn_toggle_count.setStyleSheet(
            self.toggle_style_on if self.show_count else self.toggle_style_off
        )
    ...
    # TOGGLE PERSENTASE
    # ======================================================
    def on_toggle_percentage(self):
        self.show_percentage = self.btn_toggle_pct.isChecked()
        self.btn_toggle_pct.setStyleSheet(
            self.toggle_style_on if self.show_percentage else self.toggle_style_off
        )
        if self._last_layers:
    ...
    def update_legend_multi(self, layers):
        ...
        for layer in layers:
            ...
            # ── Hitung count semua item dulu (dipakai juga utk persentase) ──
            counts = [self.get_count(layer, renderer, idx) for idx in range(len(items))]
            ...
            for idx, item in enumerate(items):
                ...
                # ── Bangun teks label sesuai toggle aktif ──
                extra_parts = []
                if self.show_count:
                    extra_parts.append(f"{count}")
                if self.show_percentage:
                    pct = (count / total_count * 100) if total_count > 0 else 0
                    extra_parts.append(f"{pct:.1f}%")

                if extra_parts:
                    label_text = f"{item.label()} ({', '.join(extra_parts)})"
                else:
                    label_text = f"{item.label()}"

                cell = QWidget()
                cell.setStyleSheet("background: transparent; border: none; outline: none;")
